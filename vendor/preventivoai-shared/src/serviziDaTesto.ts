...
function parseRigaTabellare(riga: string): ServizioDaTesto | null {
...
function parseRigaLibera(riga: string): ServizioDaTesto {
...
export function serviziDaTesto(testoServizi: string): ServizioDaTesto[] {
  return testoServizi
    .split('\n')
    .map((riga) => riga.trim())
    .filter(Boolean)
