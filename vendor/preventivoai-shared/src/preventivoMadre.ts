export type PreventivoMadreLike = {
  titolo?: string | null
...
export function labelPreventivoMadre(p: PreventivoMadreLike) {
  const titolo = titoloPreventivoMadre(p)
  const data = dataPreventivoMadre(p)
  return data ? `${titolo} · ${data}` : titolo
}

/** Rimuove la data (e eventuale versione) dal nome piano salvato in DB. */
...
export function nomePianoDaPreventivo(p: PreventivoMadreLike, tipo: 'canone' | 'rate') {
  const base = labelPreventivoMadre(p)
