...
export type AbbonamentoStatoPiano = {
  tipo: string | null
  numero_mensilita?: number | null
}

export type RataStatoPiano = {
  mese: number
  anno: number
  importo: number
...
function ordinaRate(a: RataStatoPiano, b: RataStatoPiano) {
  return a.anno - b.anno || a.mese - b.mese
}

function labelScadenza(rata: RataStatoPiano) {
  return `${MESI_BREVI[rata.mese - 1]} ${rata.anno}`
}

export function analizzaStatoPiano(abbonamento: AbbonamentoStatoPiano, rate: RataStatoPiano[]): AnalisiPiano {
  ...
  const residuo = importoPiano - importoRaccolto
  const metricheImporto = { importoRaccolto, residuo }
...
export function ordinaPianiPerStato<T extends { id: string }>(
  piani: T[],
  ratePerPiano: Record<string, RataStatoPiano[]>,
  abbonamentoById: (id: string) => AbbonamentoStatoPiano | undefined,
): T[] {
