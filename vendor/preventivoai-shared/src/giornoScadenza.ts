...
/** Giorno di scadenza piano: fallback se assente o non valido (legacy / DB nullable). */
export function giornoScadenzaEffettivo(giorno: number | null | undefined, fallback = 1): number {
  if (giorno != null && giorno >= 1 && giorno <= 31) return giorno;
  return fallback;
}
