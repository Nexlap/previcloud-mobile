...
export function tokenVariabileMessaggio(varName: string): string {
  return varName.startsWith('{') ? varName : `{${varName}}`
}

export function variabileBloccataInTemplate(
  template: string,
  tipo: TipoMessaggioCliente,
  varName: string,
): boolean {
  if (varName !== 'url') return false
  if (tipo !== 'firma_invio' && tipo !== 'firma_reminder') return false
  return template.includes('{url}')
}

export function inserisciVariabileAtIndex(
  template: string,
  varName: string,
  index: number,
  _variabiliAmmesse: string[],
): string {
  const token = tokenVariabileMessaggio(varName)
  if (template.includes(token)) return template
  const clamped = Math.max(0, Math.min(index, template.length))
  return `${template.slice(0, clamped)}${token}${template.slice(clamped)}`
}

export const CHIP_VAR_CLASS =
  'inline-block align-baseline rounded-md border border-sky-200 bg-sky-50 px-2 py-0.5 text-xs font-semibold text-sky-700 mx-0.5 select-none'

export const CHIP_VAR_LOCKED_CLASS =
  'inline-block align-baseline rounded-md border border-brand-teal/30 bg-brand-teal/10 px-2 py-0.5 text-xs font-semibold text-brand-teal mx-0.5 select-none'

export function creaChipElement(varName: string, locked: boolean): HTMLSpanElement {
  const span = document.createElement('span')
  span.contentEditable = 'false'
  span.dataset.var = varName
  if (locked) span.dataset.locked = 'true'
  span.className = locked ? CHIP_VAR_LOCKED_CLASS : CHIP_VAR_CLASS
  span.textContent = `{${varName}}`
  return span
}

function appendTestoConNewline(root: HTMLElement, text: string) {
...
export function popolaEditorMessaggio(
  root: HTMLElement,
  template: string,
  ...
  root.innerHTML = ''
  const segments = parseMessaggioSegmenti(template, variabiliAmmesse)
  for (const seg of segments) {
    if (seg.type === 'text') {
      appendTestoConNewline(root, seg.value)
    } else {
      root.appendChild(
        creaChipElement(seg.name, variabileBloccataInTemplate(template, tipo, seg.name)),
      )
    }
  }
}

export function serializzaMessaggioEditor(root: HTMLElement): string {
  let out = ''
...
const VARIABILI_POSIZIONE_BLOCCATA: Partial<Record<TipoMessaggioCliente, string[]>> = {
  firma_invio: ['url'],
  firma_reminder: ['url'],
}

/** Blocca modifiche che rompono o rimuovono parzialmente una variabile protetta. */
export function proteggiModificaMessaggio(
  ...
  for (const token of variabiliAmmesse) {
    ...
    const bloccaRimozione = tipo ? (VARIABILI_POSIZIONE_BLOCCATA[tipo] ?? []).includes(name) : false
    if (bloccaRimozione && precedente.includes(token) && !nuovo.includes(token)) return precedente
