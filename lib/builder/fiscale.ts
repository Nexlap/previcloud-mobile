import { RisultatoFiscale, TrasfertaBuilder } from './types'
...
export function calcolaTotaleVoci(voci: VocePreventivo[]) {
  return voci.reduce((acc, v) => acc + (parseFloat(v.costo) || 0) * (parseFloat(v.quantita) || 1), 0)
}

export function calcolaTotaleTrasferte(trasferte: TrasfertaBuilder[]) {
  return trasferte.reduce((acc, t) => acc + (parseFloat(t.importo) || 0), 0)
}
