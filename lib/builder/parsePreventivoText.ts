...
import { parseImportoEuro } from '../utils/importo'
...
export function collegaVociAlListino(voci: VocePreventivo[], servizi: Servizio[]): VocePreventivo[] {
  if (servizi.length === 0) return voci

  return voci.map(voce => {
    if (servizi.some(s => s.id === voce.servizio_id)) return voce

    const match = servizi.find(s => normalizzaNome(s.nome) === normalizzaNome(voce.nome))
    if (match) {
      return { ...voce, servizio_id: match.id, unita: voce.unita || match.unita }
    }
...
export function trovaMetodoPagamentoDaNome<T extends MetodoPagamento>(metodi: T[], pagamentoNome: string): T | null {
